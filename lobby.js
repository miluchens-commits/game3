...
window.LobbySystem = (function() {
  ...
  function _connectLobby(){
    ...
    _ws.onmessage=function(e){
      ...
      try{
        switch(msg.type){
          case 'lobby_state':_stateReceived=true;if(_stateTimer){clearTimeout(_stateTimer);_stateTimer=null;}console.log('[Lobby] state players:',JSON.stringify(msg.players.map(function(p){return p.clientId;})),'_localClientId=',_localClientId);msg.players.forEach(function(p){console.log('[Lobby] check p.clientId=',p.clientId,' !== ',_localClientId,'=',p.clientId!==_localClientId);if(p.clientId!==_localClientId)addRemotePlayer(p);});if(msg.players.length>0){var otherCount=msg.players.filter(function(p){return p.clientId!==_localClientId}).length;if(otherCount>0){var el=document.getElementById('lobby-msg');if(el){el.innerHTML='🟢 '+otherCount+' 人在大廳';el.style.display='block';}}}if(msg.players.length===0)console.log('[Lobby] state empty!');break;
          case 'lobby_player_join':if(msg.clientId!==_localClientId){addRemotePlayer(msg);var cnt=Object.keys(_remotePlayers).length;var el2=document.getElementById('lobby-msg');if(el2){el2.innerHTML='🟢 '+cnt+' 人在大廳';el2.style.display='block';}var notifEl=document.getElementById('lobby-notif');if(notifEl){notifEl.textContent='🟢 '+(msg.name||msg.clientId.substr(0,6))+' 進入大廳';notifEl.classList.add('show');_notifT=0;}}break;
          ...
          case 'lobby_player_leave':removeRemotePlayer(msg.clientId);var cnt2=Object.keys(_remotePlayers).length;var el3=document.getElementById('lobby-msg');if(el3){if(cnt2<1){el3.style.display='none';}else{el3.innerHTML='🟢 '+cnt2+' 人在大廳';}}break;
        }
